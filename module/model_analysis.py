...
def model_analysis_2_2(score_df,place_name,version):
    #基準をクリアしたモデルを格納するdf
    #depths_arr=[5,8]
    depths_arr=[5,7]
    #th_arr=[0.85,0.9,0.92]
...
def ym_analysis_rondom(result_base_df,use_model_df,place_name,version,year):
    #渡した会場と年ごとに、今あるモデルっを使った時の利益率や的中数のシミュレーションを行う関数
    #探査結果から学習したモデルを保存する関数、
    print(place_name)
    #==============================================================================
    ...
    for com, model in models_dict.items():
        #pred_df['pred_{}'.format(int(com))]=model.predict(test)
        predict_y_test_proba_arr = model.predict_proba(target_x_test)#まだ多次元リスト
        predict_y_test=[proba_arr[1] for proba_arr in predict_y_test_proba_arr]#1にあたる部分の確率のみ出力
        pred_df['pred_{}'.format(int(com))]=predict_y_test
...
def check_model_V2_2(result_base_df,use_model_df,place_name,version):#モデルをすべてpickleで読み込んで精度検証、
    #探査結果から学習したモデルを保存する関数、
    print(place_name)
    #==============================================================================
    #学習関数で場所ごとにバージョンに対応した学習データを作る
    ...
    for index, model_row in use_model_df.iterrows():
        ...
        train_x, valid_x, train_y, valid_y = train_test_split(target_x_train, target_y_train, test_size=0.2,shuffle=True, random_state=7)#学習データ内でさらに分割してロスをもとに修正をする。
        pickle_path="../bot_database/{place_name}/model_pickle_{place_name}/com{com}_{depth}_{target_per}_{th}_{place_name}_{V}.sav".format(place_name=place_name,com=result_com,depth=depth,target_per=target_per,th=th,V=version)#モデルを保存
        RF = pickle.load(open(pickle_path, 'rb'))
        # 未知データに対する予測値
        #predict_y_test = RF.predict(target_x_test)
